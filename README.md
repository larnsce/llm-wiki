...
| `wiki-paper` | One hub page per manuscript under `wiki/papers/<slug>` (issue #146): scaffold with the six-section skeleton, attach existing literature/data pages as links (append-only on both sides), read-only status; the hub is the published paper site's table of contents and the export walk's root |
