...
# No agent definitions ship (REQ-807 retired with model tiering, #108):
# the install must not create an agents directory.
run test -e "$WORK/tier-default/.claude/agents"
assert_exit 1 "setup: default install creates no agents directory (REQ-807 retired)"
run bash "$REPO_ROOT/setup.sh" --project "$WORK/tier-symlink" --symlink
assert_exit 0 "setup: --symlink install runs clean"
run test -L "$WORK/tier-symlink/.claude/skills/wiki-ingest"
assert_exit 0 "setup: --symlink links skill directories (REQ-802)"
